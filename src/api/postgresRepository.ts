...
function policyFromRow(row: SystemPolicyRow): SystemPolicy {
  return {
    ...
    leaveReasonRequired: row.leave_reason_required === undefined ? defaultSystemPolicy.leaveReasonRequired : Boolean(row.leave_reason_required),
...
function policyToRow(settings: Partial<SystemPolicy>): DbRow {
  return {
    ...
    leave_reason_required: settings.leaveReasonRequired,
