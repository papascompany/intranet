...
**모바일 실전 규칙 (M4 필수, 설계 검증 반영)**
1. **인앱브라우저 분기**: 카카오톡·네이버·인스타그램 인앱 UA 감지 시 결제 직전 `기본 브라우저에서 계속하기` 시트 노출(카드사 앱 스킴 `intent://`·`ispmobile://` 복귀 실패 예방). 앱별 외부 브라우저 전환 스킴 대응표를 M4에서 작성·유지하고, 복귀 시 세션 토큰 승계는 docs/05 §4.1 이중화 규칙을 따른다.
2. **간편결제 재사용**: `customerKey = sessionId` — 같은 세션의 2차 주문부터 결제 포함 ≤ 15s(추가주문율 방어).
3. **SDK prefetch**: 선결제 매장은 카트 1건째 담기는 순간 위젯 SDK를 미리 로드(체감 1~3s 단축, 동적 import).
4. **실패 복귀**: 위젯 이탈/승인 실패/10분 만료 → 카트 보존 상태로 장바구니 시트 복귀 + `결제 다시 시도`/`직원 호출` 배너(docs/05 §4.9).
5. 위젯 진입 전 화면에 매장 상호·사업자명 1줄 표기(전상법 표기 겸 신뢰 신호).
